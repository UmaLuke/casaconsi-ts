...
- `ChatController` (`/api/chat`, `[Authorize]`), mismo criterio que `MatchController`/`ProfileController`: opera sobre el usuario del token, nunca sobre un id que mande el cliente.
  - `GET /api/chat` — conversaciones del usuario (una por `Match`), con nombre/foto de la contraparte (vía `IProfileRepository`, reutilizado) y preview del último mensaje. Ordenadas por fecha del último mensaje descendente; las conversaciones sin mensajes todavía quedan al final. **(2026-09-16)**
  - `GET /api/chat/{matchId}/messages` — historial completo de una conversación, orden cronológico ascendente. 403 si el usuario no es Student ni Host de ese `Match`; 400 si el `matchId` no corresponde a ningún `Match` existente (ambos casos surgen de `EnsureParticipantAsync`, vía `UnauthorizedAccessException`/`InvalidOperationException` capturadas en el controller). **(2026-09-16)**
...
- `IChatService.EnsureParticipantAsync(userId, matchId)`: valida que el usuario sea Student o Host del `Match` antes de leer/escribir. La usan tanto `ChatController` (indirectamente, dentro de `GetMessagesAsync`) como `ChatHub` — un solo lugar para esa regla.
- **`Hubs/ChatHub.cs`** (`/hubs/chat`, `[Authorize]`): sin REST para enviar mensajes, todo pasa por acá.
  - `JoinConversation(matchId)`: valida participación con `EnsureParticipantAsync` y, si es válida, suma la conexión al grupo de SignalR `match-{matchId}`.
  - `LeaveConversation(matchId)`: **corregido (2026-09-16)** — a diferencia de lo que decía esta página, `LeaveConversation` **no** vuelve a validar participación; solo hace `Groups.RemoveFromGroupAsync(...)` sobre el grupo `match-{matchId}`. Tiene sentido: sacar una conexión de un grupo al que ya pertenece no necesita repetir el chequeo de negocio.
  - `SendMessage(matchId, content)`: persiste vía `ChatService.SendMessageAsync` (que primero corre `EnsureParticipantAsync` y después valida que el contenido no esté vacío — lo trimea y tira `InvalidOperationException` si queda vacío) y hace `Clients.Group("match-{matchId}").SendAsync("ReceiveMessage", messageDto)` — le llega a todos los conectados a esa conversación, incluido quien lo mandó (así el remitente ve su propio mensaje aparecer igual que el resto, sin duplicar lógica en el frontend). **(2026-09-16)**
  - Errores de negocio (`InvalidOperationException`/`UnauthorizedAccessException`) se relanzan como `HubException` en `JoinConversation` y `SendMessage` para que el cliente los vea (si no, SignalR devuelve un error genérico sin mensaje).
...
- Estados de conexión/reconexión visibles para el usuario (hoy `isConnected` solo se usa para habilitar/deshabilitar el envío, no hay indicador visual de "conectando..."/"desconectado" — confirmado, no hay ningún elemento en el JSX que lo muestre). **(2026-09-16)**
...
- Sin indicador de mensaje enviado/entregado/leído (decisión de alcance del MVP — evaluar si se agrega más adelante).
...
- [[../decisiones/ADR-0002-persistencia-jwt|ADR-0002-persistencia-jwt]]
