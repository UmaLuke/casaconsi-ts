...
**Estado:** 🚧 Backend MVP + flujo de revisión de staff implementados end-to-end. Frontend: insignia en `ProfilePage.tsx` con los dos estados probados (Básico naranja, Alta confianza/Premium turquesa). **(2026-08-27)** Nuevo: panel Admin real para revisar y aprobar/rechazar los ítems 7-10 ("Alta Confianza") — cola + detalle, demostrado a la clienta — y las 2 primeras evidencias autocontenidas (antecedentes penales, referencias personales) visibles del lado admin. **(2026-09-16)** Corregido tras auditoría: la insignia pública para "la otra parte" **ya está implementada**, esta sección la daba como pendiente y no lo estaba — hay un tercer componente, `TrustBadgeCompact` (no documentado hasta ahora), usado en `StudentDetailPage.tsx`, `InterestedStudentsPage.tsx` y `SpaceDetailPage.tsx`, alimentado por `trustScore`/`trustLevel` (o `hostTrustScore`/`hostTrustLevel`) que `MatchService`/`SpaceService` calculan del lado del servidor llamando a `ITrustService.GetStatusAsync` — no es un endpoint público nuevo, es composición entre services (ver [[Match]], [[Space]]). Sigue sin aparecer en `DiscoverPage.tsx` (las cards de swipe) ni en `SpaceDetailsModal.tsx`. Decisiones tomadas: validación de identidad **manual** (no biometría automática, ver más abajo), diseño final de las **tres insignias** (`TrustBadge`, `TrustBadgeCompact` y `PremiumBadge`), y que los ítems 7-10 **ya no se autodeclaran** — solo pasan a `true` cuando el staff aprueba desde el panel Admin. Falta: UI para que el usuario autodeclare los ítems 1-6 (el `PUT /api/trust/items` existe en el backend pero ningún form lo llama todavía), la insignia en las cards de `DiscoverPage.tsx`/`SpaceDetailsModal.tsx`, flujo de pago Premium real, formulario del usuario para cargar la evidencia de Alta Confianza (backend ya listo), y los otros 2 ítems de evidencia (entrevista virtual, historial de convivencia — dependen de subsistemas que no existen todavía).
...
- **Capas:** `TrustController` (`/api/trust`, `[Authorize]`) → `TrustService` → `TrustRepository`, respetando la separación de siempre. Igual que `ProfileController`, opera siempre sobre el usuario del token.
  - `GET /api/trust/status` — devuelve `TrustStatusResponseDto`: `score`, `maxScore` (10), `level`, `membershipTier`, `isDemoUser`, y el detalle `items[]` (cada uno con `key`, `label`, `completed`, `requiresPremium`, `locked`). Verificado contra `TrustDtos.cs`: los campos coinciden exactamente.
  - `PUT /api/trust/items` — auto-declarativo, pero **solo para los ítems 1-6** (Básica). **(2026-08-27)** Los ítems 7-10 (Alta Confianza) se sacaron de este endpoint — antes un usuario Premium los podía marcar en `true` directamente sin ningún control; ahora `UpdateTrustItemsRequestDto` ni siquiera tiene esos 4 campos (confirmado: solo `IdentityVerified`, `ContactVerified`, `SocialMediaVerified`, `CreditStatusVerified`, `ProofOfStatusVerified`, `SwornDeclarationAccepted`), y las únicas dos formas de que pasen a `true` son que el staff los apruebe desde el panel Admin (ver más abajo) o que el usuario los vuelva a solicitar tras un rechazo. El viejo comportamiento (gateo silencioso por `MembershipTier` dentro de `UpdateItemsAsync`) queda documentado acá porque fue la razón original para no dejar que cualquier Premium se autodeclare Alta Confianza sin evidencia real.
  - `TrustService.BuildResponse` calcula `score` (cuenta de ítems en `true`) y `level` según los umbrales del docx (0 → `sin_verificar`, 1-6 → `basico`, 7-10 → `alta_confianza`; confirmado en el `switch` del código).
...
### Backend
- **Enum nuevo** `Models/Enum/VerificationReviewStatus.cs`: `NoSolicitado`/`Pendiente`/`Aprobado`/`Rechazado`, mismo patrón de `JsonConverter` a snake_case que `MembershipTier`/`VerificationLevel`. Verificado: el converter serializa `no_solicitado`/`pendiente`/`aprobado`/`rechazado`.
...
- **`TrustController`** — endpoint `POST /api/trust/solicitar-alta-confianza` (`RequestAltaConfianzaAsync`): el usuario Premium dispara la solicitud (pasa de `NoSolicitado` a `Pendiente`, guarda `RequestedAtUtc`, y falla con `InvalidOperationException` si no es Premium o si ya hay una solicitud `Pendiente`). No hace nada por sí solo — es lo que hace aparecer al usuario en la cola del admin.
- **`ITrustRepository`/`TrustRepository`** — método `GetPendingAltaConfianzaAsync()`: trae todas las `ProfileVerification` con `AltaConfianzaStatus == Pendiente`, ordenadas por `AltaConfianzaRequestedAtUtc`, incluyendo el `User` relacionado (para nombre/rol/tier sin N+1 queries en la cola).
- **Capas nuevas, mismo patrón Controller → Service → Repository de siempre:**
  - `DTOs/Admin/AdminVerificationDtos.cs`: `AdminVerificationQueueItemDto` (fila de la cola: `userId`, `name`, `role`, `basicScore`, `requestedAtUtc`), `AdminVerificationItemDto` (`key`/`label`/`completed`), `AdminVerificationDetailDto` (detalle completo), `AdminVerificationRejectRequestDto` (`{ reason: string }`, campo `Reason` requerido).
  - `IAdminVerificationService`/`AdminVerificationService` (**reusa `ITrustRepository`** — no tiene repository propio, mismo criterio que `AuthService`/`AccountService` reusan `UserManager` en vez de tener su capa de datos):
  ...
  - `AdminVerificationsController` (`/api/admin/verificaciones`, `[Authorize(Roles = "Admin")]`): `GET /` (cola), `GET /{userId}` (detalle), `POST /{userId}/aprobar`, `POST /{userId}/rechazar` (body `AdminVerificationRejectRequestDto`). Endpoints verificados contra el código, coinciden exactamente.
- **DI:** `IAdminVerificationService`/`AdminVerificationService` registrado en `Program.cs` junto a los demás `AddScoped`.

### Frontend
- `types/admin.ts`: `AdminVerificationStatus`, `AdminVerificationQueueItem`, `AdminVerificationItem`, `AdminVerificationDetail` — alineados con los DTOs de arriba (verificado campo a campo).
- `services/adminService.ts`: `getVerificationQueue`, `getVerificationDetail`, `approveVerification`, `rejectVerification` — vía `apiFetch`.
- `pages/admin/VerificationsQueuePage.tsx` (ruta `/dashboard/verificaciones`): tabla de la cola, con el mismo shell de sidebar que `DashboardPage.tsx`.
- `pages/admin/VerificationDetailPage.tsx` (ruta `/dashboard/verificaciones/:userId`): score consolidado (básico confirmado + Alta Confianza pendiente/aprobado), lista de los 6 ítems básicos (sin acción, solo lectura), lista de los 4 ítems de Alta Confianza con su estado, tarjeta de evidencia (ver sección siguiente), botones **Aprobar**/**Rechazar** (rechazar abre un modal que pide motivo, obligatorio).
- `App.tsx`: las dos rutas nuevas están bajo `<ProtectedRoute requireAdmin>` (verificado).
- `DashboardPage.tsx`: el botón "Verificaciones" del sidebar pasó a ser un link a `/dashboard/verificaciones`; la tarjeta "Verificaciones pendientes" carga la cola real (no se pudo re-verificar el detalle exacto de este archivo en esta auditoría porque no estaba en el alcance de código revisado — se deja tal como estaba documentado).
- **Bug encontrado y corregido de paso:** `FloatingNav.tsx` reaparecía en las páginas admin nuevas — su lista `HIDDEN_ROUTES` comparaba con `===` exacto contra rutas fijas, así que no matcheaba `/dashboard/verificaciones` ni `/dashboard/verificaciones/:userId`. Verificado en el código actual: `FloatingNav.tsx` ahora usa `location.pathname.startsWith('/dashboard')` para ocultarse en toda la sección de dashboard, además de la lista `HIDDEN_ROUTES` para el resto de rutas fijas (`/`, `/cuestionario/buscar`, `/cuestionario/ofrecer`).
...
> **Nota de esta auditoría (2026-09-16):** `DemoProfileSeeder.cs` no estaba entre los archivos de código relevados en esta pasada (aunque otros archivos del backend sí lo referencian, así que existe), por lo que esta sección y la tabla de "Datos de prueba" más abajo no se pudieron re-verificar línea por línea. Se dejan intactas tal como estaban; si hubo cambios en el seeder no reflejados acá, van a quedar pendientes de una próxima auditoría.
...
|---|---|---|---|
| 7 | Referencias personales | ✅ sí (form simple) | **Construido en el backend** |
| 9 | Antecedentes penales | ✅ sí (upload de archivo) | **Construido en el backend** |
...
**(2026-09-16)** Aclaración: "construido" acá se refiere solo al backend (los 3 endpoints de `/api/trust/alta-confianza/*` existen y funcionan, verificado en `TrustController.cs`/`TrustService.cs`). El formulario del lado del usuario para cargar esta evidencia **sigue sin existir** — ver "Pendiente" más abajo, todavía vigente.
...
### Backend
- **`ProfileVerification`** — 8 columnas nuevas: `Reference1Name`/`Reference1Phone`/`Reference1Relationship`, `Reference2Name`/`Reference2Phone`/`Reference2Relationship` (campos planos, no una tabla separada — la cantidad de referencias es fija en 2, no justifica una entidad EF Core nueva), `CriminalRecordDocumentPath`, `CriminalRecordDocumentUploadedAtUtc`. Verificado contra `ProfileVerification.cs`: coincide exactamente.
- **`DTOs/Trust/TrustDtos.cs`**: `SavePersonalReferencesRequestDto` (los 6 campos de arriba, todos `required string`), `AltaConfianzaEvidenceDto` (para que el usuario vea lo que ya cargó: las 6 referencias como `string?`, `HasCriminalRecordDocument`, `CriminalRecordDocumentUrl`, `CriminalRecordDocumentUploadedAtUtc`).
- **`ITrustService`/`TrustService`** — 3 métodos, con `IFileStorageService` sumado como dependencia del constructor:
  ...
  - `UploadCriminalRecordDocumentAsync(userId, file)` → valida extensión (`AllowedDocumentExtensions` = pdf/jpg/jpeg/png) y reusa `IFileStorageService.SaveAsync(file, "verificaciones/{userId}")`.
- **`TrustController`** — 3 endpoints: `GET /api/trust/alta-confianza/evidencia`, `PUT /api/trust/alta-confianza/referencias`, `POST /api/trust/alta-confianza/antecedentes-documento` (multipart, recibe `IFormFile file` directo). Verificados contra el código, coinciden exactamente.
- **`DTOs/Admin/AdminVerificationDtos.cs`**: `AdminVerificationReferenceDto` (`Name`/`Phone`/`Relationship`, todos requeridos) + 4 campos en `AdminVerificationDetailDto` (`Reference1`, `Reference2`, `CriminalRecordDocumentUrl`, `CriminalRecordDocumentUploadedAtUtc`).
...
### Frontend (solo lado admin, sigue así)
- `types/admin.ts`: `AdminVerificationReference` (`{ name, phone, relationship }`) + los 4 campos en `AdminVerificationDetail`. El campo `relationship` existe porque el requisito del cliente para las referencias es "nombre + número de celular + vínculo de la persona" — le da contexto al admin sobre cómo esa persona conoce al solicitante (ex-conviviente, familiar, amigo), no afecta ningún cálculo de puntaje.
- `VerificationDetailPage.tsx`: tarjeta "Evidencia cargada por el usuario" — grid de las 2 referencias (o un placeholder punteado "sin cargar" si falta alguna) + link al documento de antecedentes (`resolveUploadedFileUrl`, mismo helper que ya arma la URL del avatar en `utils/avatar.ts`) con fecha de subida. Si no hay nada cargado todavía muestra "Todavía no cargó ninguna evidencia" — esperable mientras no exista el formulario del usuario.

### Pendiente — el paso inmediato siguiente
**El formulario del lado del usuario no está construido todavía — confirmado en esta auditoría (2026-09-16).** Los 3 endpoints del backend ya existen y funcionan, pero `services/trustService.ts` solo tiene `getTrustStatus` — no hay ninguna función `getAltaConfianzaEvidence`/`savePersonalReferences`/`uploadCriminalRecordDocument`, ni ninguna pantalla que llame a esos 3 endpoints (se buscó en todo `frontend/src` y no aparecen). Tampoco hay subida real de archivo para DNI, selfie o constancia de domicilio (ítems 1, 2 y 5) — de los 10 ítems, el único con upload de archivo real conectado de punta a punta es el certificado de antecedentes penales, y solo desde Postman/base de datos, no desde una pantalla de usuario. Candidato natural para el formulario: `ProfilePage.tsx`, sin confirmar todavía. Hasta que esto exista, la única forma de generar datos de prueba en la cola es sembrarlos directo en la base (ver Gotcha de reseteo de Sofía más arriba) o cargarlos a mano vía Postman.

## Frontend (implementado, parcial)
- `types/trust.ts`: `MembershipTier`, `VerificationLevel`, `TrustItem`, `TrustStatus` — alineados con los DTOs de arriba (verificado campo a campo).
- `services/trustService.ts`: **una sola función**, `getTrustStatus(token)` → `GET /api/trust/status`. Confirmado en esta auditoría (2026-09-16): sigue siendo el único endpoint de Trust con consumidor en el frontend — ningún form llama a `PUT /api/trust/items` ni a los 3 endpoints de `/alta-confianza/*` (ver "Pendiente" en la sección de evidencia).

### Insignias del cliente — diseño final (2026-08-06, ampliado 2026-09-16)
Lucía pidió que hubiera **dos** insignias distintas — una para Básico y otra para Premium — en vez de una sola. El diseño pasó por varias vueltas (anillo cappeado a 6, layout en columna con texto debajo, ícono premium metido en un círculo relleno) hasta llegar a esto, que es lo que quedó aprobado. **(2026-09-16)** Auditoría encontró un tercer componente, `TrustBadgeCompact`, que no estaba documentado — son en total tres:

- `components/common/TrustBadge.tsx`: **el anillo grande, para la vista del propio perfil.** Sigue siendo de **0 a 10** (el cap a 6 que se probó en el medio se descartó — Lucía prefirió mantener la escala completa siempre visible). Color del anillo por nivel, con los colores de marca reales del proyecto (`index.css`: `--color-brand-orange: #FF7A00`, `--color-brand-teal: #00B4C4` — no los genéricos `warning`/`success` de DaisyUI):
...
- `components/common/TrustBadgeCompact.tsx` **(2026-09-16, existía en el código pero no estaba en el doc):** versión mini — un círculo con **borde** de color (no relleno de progreso como el anillo grande) y el puntaje adentro (`{score}/{maxScore}`), con el label al costado (opcional vía `hideLabel`). Es la insignia que se le muestra a "la otra parte" del match, no al dueño del perfil. Usos confirmados en el código:
  - `pages/StudentDetailPage.tsx` — el anfitrión ve el `trustScore`/`trustLevel` de la persona interesada en el detalle, con label.
  - `pages/InterestedStudentsPage.tsx` — mismo dato, versión sin label (`hideLabel`) superpuesta arriba a la derecha de la foto en cada card de la lista.
  - `pages/SpaceDetailPage.tsx` — la persona que busca ve el `hostTrustScore`/`hostTrustLevel` del anfitrión, con label.
  - Ninguno de estos tres puntos llama a `trustService.ts`: el dato ya viene embebido en la respuesta de `matchService.ts` (`StudentDetail`, `InterestedStudent`) y `spaceService.ts` (`Space`) — ver "Interacciones" más abajo, `MatchService`/`SpaceService` (backend) inyectan `ITrustService` y llaman `GetStatusAsync` al armar esos DTOs.
  - **Todavía no** aparece en `pages/DiscoverPage.tsx` (las cards de swipe/like) ni en `components/features/spaces/SpaceDetailsModal.tsx` — confirmado por lectura de esos dos archivos, no importan `TrustBadgeCompact` ni ningún campo de trust.
...
- `pages/ProfilePage.tsx`, pestaña "Datos de cuenta" (`AccountTab`): renderiza `TrustBadge` + `PremiumBadge` apilados (`flex-col items-end gap-2`) a la derecha del avatar.
- **Todavía no implementado:** UI para que el usuario marque los ítems 1-6 (el `PUT /api/trust/items` existe en el backend pero ningún form lo llama todavía). **(2026-09-16)** La insignia pública para la otra parte del match **ya está implementada** (ver `TrustBadgeCompact` arriba) — lo que falta puntualmente es agregarla en `DiscoverPage.tsx`/`SpaceDetailsModal.tsx`, no construirla de cero como decía esta sección antes de esta auditoría.
...
**(2026-09-16) Composición server-to-server, no documentada hasta ahora:** `MatchService` y `SpaceService` **no llaman a `TrustController` por HTTP** — inyectan `ITrustService` directamente y llaman `GetStatusAsync(userId)` al armar sus propios DTOs, para exponer el nivel de confianza de "la otra parte" sin duplicar la lógica de cálculo de score:
- `MatchService` → `TrustScore`/`TrustLevel` en `InterestedStudent` y `StudentDetail` (usado por `InterestedStudentsPage.tsx`/`StudentDetailPage.tsx`, ver [[Match]]).
- `SpaceService` → `HostTrustScore`/`HostTrustLevel` en `Space` (usado por `SpaceDetailPage.tsx`, ver [[Space]]).
...
|---|---|---|
| `pages/ProfilePage.tsx` (`AccountTab`) | `getTrustStatus` | Renderiza `TrustBadge` + `PremiumBadge` junto al avatar (perfil propio) |
...
| `pages/StudentDetailPage.tsx` **(2026-09-16)** | ninguna de `trustService` — dato embebido en `matchService.getInterestedStudentDetail` | `TrustBadgeCompact` con el `trustScore`/`trustLevel` del/la estudiante interesado/a, para el anfitrión |
| `pages/InterestedStudentsPage.tsx` **(2026-09-16)** | ninguna de `trustService` — dato embebido en `matchService.getInterestedStudents` | `TrustBadgeCompact` compacto (`hideLabel`) sobre la foto de cada card |
| `pages/SpaceDetailPage.tsx` **(2026-09-16)** | ninguna de `trustService` — dato embebido en `spaceService.getSpaceById` | `TrustBadgeCompact` con `hostTrustScore`/`hostTrustLevel`, para quien busca alojamiento |
...
SpaceDetailPage.tsx (al montar)                                    ← agregado 2026-09-16
  → getSpaceById(id)                              [spaceService.ts]
    → apiFetch('/api/spaces/{id}')                  [httpClient.ts]
      → SpaceService (backend) → _trustService.GetStatusAsync(space.HostUserId)
    ← Space { ..., hostTrustScore, hostTrustLevel }
  → <TrustBadgeCompact score={space.hostTrustScore} level={space.hostTrustLevel} ... />
```
...
> Ver nota de esta auditoría más arriba (sección "Panel Admin" → "Datos de prueba para esto"): `DemoProfileSeeder.cs` no estuvo entre los archivos revisados en esta pasada, así que la tabla siguiente no se pudo re-verificar contra el código y se deja tal como estaba.

Estado (según la última revisión registrada, 2026-08-27) de `Students`/`Hosts` en `DemoProfileSeeder.cs`:
...
- **(2026-09-16) Componente sin documentar encontrado en auditoría:** `TrustBadgeCompact.tsx` existe en el código y está integrado en tres pantallas reales (`StudentDetailPage.tsx`, `InterestedStudentsPage.tsx`, `SpaceDetailPage.tsx`) desde antes de esta fecha, pero nunca se agregó a este documento — la sección "Todavía no implementado" seguía diciendo que la insignia pública no existía. Moraleja para el mantenimiento del vault: cuando se agrega un componente nuevo que resuelve una "Pregunta abierta" o un ítem de "Falta", conviene actualizar esta página en el mismo momento, no después.
...
- Diseño de la insignia pública (Discover/Explorar): ¿mismo `TrustBadge` compacto, o una versión mini solo con el nivel (sin el detalle de los 10 ítems)? **Resuelto (2026-09-16):** existe `TrustBadgeCompact`, una versión mini con borde de color y el puntaje adentro, sin el detalle de los 10 ítems — se usa en `StudentDetailPage.tsx`, `InterestedStudentsPage.tsx` y `SpaceDetailPage.tsx`. Sigue sin usarse en `DiscoverPage.tsx` (las cards de swipe) ni en `SpaceDetailsModal.tsx` — eso sí queda abierto.
...
- [[Match]] — **(2026-09-16)** `MatchService` inyecta `ITrustService` y compone `trustScore`/`trustLevel` en `InterestedStudent`/`StudentDetail`, consumido por `TrustBadgeCompact` en `InterestedStudentsPage.tsx`/`StudentDetailPage.tsx`.
- [[Space]] — **(2026-09-16)** mismo patrón: `SpaceService` compone `hostTrustScore`/`hostTrustLevel` en `Space`, consumido por `TrustBadgeCompact` en `SpaceDetailPage.tsx`.
