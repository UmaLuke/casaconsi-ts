...
- **`LoginModal.tsx`, redirect post-login: resuelto (verificado 2026-09-16).** Ya tiene una rama propia para `advisor` (host → `/descubrir`, advisor → `/asesorias`, student → `/explorar`) — no es el bug binario que describía esta entrada; quedó desactualizada la documentación, no el código. Ver [[modulos/Auth]].
