...
| `matchService.ts` | `GET /api/match`, `GET /api/match/feed`, `POST /api/match/like`, `GET /api/match/interested`, `GET /api/match/interested/{studentUserId}` **(2026-09-16)** | `pages/MessagesPage.tsx` (`getMatches`), `pages/DiscoverPage.tsx` (`getFeed` + `registerLikeDecision`), `pages/ExploreSpacesPage.tsx` y `components/features/landing/ExploreSpaces.tsx` (`registerLikeDecision`, botones ✕/✓ sobre un `Space`), `pages/InterestedStudentsPage.tsx` (`getInterestedStudents`) y `pages/StudentDetailPage.tsx` (`getInterestedStudentDetail` + `registerLikeDecision`) — ver [[../modulos/Match\|Match]] |
