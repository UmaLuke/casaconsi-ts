...
export const Header = () => {
  ...
  // Extraemos los datos del usuario y la función de logout de tu contexto global
  const { user, logout } = useAuth();

  // Efecto para detectar el scroll de la ventana y cambiar el estado
  useEffect(() => {
    const handleScroll = () => {
      // Si bajamos más de 20px, el navbar se vuelve sólido
  ...
  // Variables para facilitar la lectura de las clases dinámicas de texto
    ...
    >
      <div className="container mx-auto navbar px-4 md:px-7">
        ...
        <div className="navbar-end gap-2 sm:gap-4 items-center">
          ...
          {/* VERIFICACIÓN DE USUARIO: Mostramos el perfil o los botones */}
          {user ? (
            <div className="dropdown dropdown-end">
              <div tabIndex={0} role="button" className="btn btn-ghost btn-circle avatar transition-transform hover:scale-105">
                <div className="w-9 sm:w-10 rounded-full ring ring-brand-teal ring-offset-base-100 ring-offset-2">
                  {/* Si el usuario no tiene foto, la API de ui-avatars le genera una con sus iniciales */}
              ...
              <ul tabIndex={0} className="mt-3 z-[1] p-2 shadow-lg menu menu-sm dropdown-content bg-base-100 rounded-box w-52 text-base-content border border-base-200">
                <li className="menu-title px-4 py-2">
                  ...
                  <span className="text-xs font-normal text-base-content/60 capitalize block">{user.role}</span>
                </li>
            ...
            <>
              {/* Botón de Iniciar Sesión (Abre el Modal) */}
              <button 
              ...
              {/* Botón de Registrarse (Lleva a la página de registro) */}
              <Link 
                to="/register" 
      ...
      {/* Instanciamos el Modal aquí para que esté disponible en todo el layout */}
      <LoginModal />
